...
CFG = {
    ...
    'HARD_NEGATIVE_RATIO': 0.2,  # 배치에서 하드 네거티브 샘플의 비율
    'HARD_NEGATIVE_MEMORY_SIZE': 1000  # 하드 네거티브 샘플을 저장할 최대 크기
}
...
class CustomDataset(Dataset):
    def __init__(self, img_path_list, label_list, transforms=None, save_image=False):
    ...
    def __getitem__(self, index):
        ...
        if self.save_image:
            self.save_img(image, index)
        if self.label_list is not None:
            label = self.label_list[index]
            return image, label, index  # 인덱스 추가
        else:
            return image
    ...
    def save_img(self, image, index):
        ...
        image_bgr = image[..., ::-1] if image.shape[-1] == 3 else image
        os.makedirs("./saved_image", exist_ok=True)
        save_path = f"./saved_image/transformed_{index}.jpg"
...
def train(model, optimizer, train_loader, val_loader, scheduler, device, class_names, hard_negative_miner=None, best_score=0, cur_epoch=1, experiment_name="base", folder_path="base", accumulation_steps=1):
    model.to(device)
    # 기본 CrossEntropyLoss는 배치에 대한 평균 손실값만 반환합니다
    criterion = nn.CrossEntropyLoss(reduction='none')  # reduction='none'으로 설정하여 샘플별 손실값 반환
...
if __name__ == '__main__':
    ...
    wandb.config.update({
        ...
        "model": model_name,
        "hard_negative_ratio": CFG['HARD_NEGATIVE_RATIO'],
        "hard_negative_memory_size": CFG['HARD_NEGATIVE_MEMORY_SIZE'],
        "balanced_class_sampling": True  # 클래스 균형 샘플링 적용 여부 추가
    })
    ...
    config['train']['hard_negative_ratio'] = CFG['HARD_NEGATIVE_RATIO']
    config['train']['hard_negative_memory_size'] = CFG['HARD_NEGATIVE_MEMORY_SIZE']
    ...
    if trained_path != "":
        ...
        infer_model = train(model, optimizer, train_loader, val_loader, scheduler, device, class_names, 
                           hard_negative_miner=hard_negative_miner, best_score=best_score, 
                           cur_epoch=start_epoch, experiment_name=experiment_name, folder_path=folder_path, accumulation_steps=CFG['ACCUMULATION_STEPS'])
    else:
        infer_model = train(model, optimizer, train_loader, val_loader, scheduler, device, class_names, 
                           hard_negative_miner=hard_negative_miner, experiment_name=experiment_name, folder_path=folder_path, accumulation_steps=CFG['ACCUMULATION_STEPS'])
