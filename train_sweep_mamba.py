...
def train(model, optimizer, train_loader, val_loader, scheduler, device, class_names, best_score=0, cur_epoch=1, experiment_name="base", folder_path = "base", accumulation_steps=8):
    model.to(device)
...
def train_single_run():
    ...
    experiment_name = f"{model_name.replace('.','_')}_lr{CFG['LEARNING_RATE']:.2e}"
    ...
    df, _, _, _ = train_test_split(df, df['rock_type'], test_size=0.96, stratify=df['rock_type'], random_state=CFG['SEED'])
...
def main():
    ...
    parser.add_argument('--project', type=str, default='sweep_mambaout', help='wandb project name')
    ...
    parser.add_argument('--config', type=str, default="./sweepmamba.yaml", help='sweep configuration file (optional)')
