...
class ProgressiveScheduler:
    """Hard negative ratio를 점진적으로 조정하는 스케줄러"""
    
    def __init__(self, initial_ratio=0.0, final_ratio=0.5, total_epochs=100, schedule_type='linear'):
        """
        Args:
            initial_ratio: 초기 hard negative ratio
            final_ratio: 최종 hard negative ratio
            total_epochs: 전체 epoch 수
            schedule_type: 스케줄링 방식 ('linear', 'exponential', 'cosine', 'step')
        """
    ...
    def get_ratio(self, current_epoch):
        """현재 epoch에 대한 hard negative ratio 반환"""
        ...
        if self.schedule_type == 'linear':
            ...
            # 지수적 증가
            ratio = self.initial_ratio + (self.final_ratio - self.initial_ratio) * (progress ** 2)
            
        elif self.schedule_type == 'cosine':
            # Cosine annealing 스타일
            ratio = self.initial_ratio + (self.final_ratio - self.initial_ratio) * (1 - math.cos(progress * math.pi / 2))
            
        elif self.schedule_type == 'step':
            # Step-wise 증가 (25%, 50%, 75%에서 단계적 증가)
            if progress < 0.25:
                ratio = self.initial_ratio
            elif progress < 0.5:
                ratio = self.initial_ratio + (self.final_ratio - self.initial_ratio) * 0.25
            elif progress < 0.75:
                ratio = self.initial_ratio + (self.final_ratio - self.initial_ratio) * 0.5
            else:
                ratio = self.initial_ratio + (self.final_ratio - self.initial_ratio) * 0.75
                
        else:
            raise ValueError(f"Unknown schedule_type: {self.schedule_type}")
            
        return max(0.0, min(1.0, ratio))  # 0과 1 사이로 클리핑

class ProgressiveBalancedHardNegativeBatchSampler(Sampler):
    def __init__(self, dataset_size, batch_size, hard_negative_miner, labels, num_classes, 
                 progressive_scheduler, accumulation_steps=1, current_epoch=0):
        """
        Progressive hard negative ratio를 사용하는 샘플러
        ...
        Args:
            ...
            labels: 각 샘플의 클래스 레이블 (numpy array)
            num_classes: 클래스 수
            progressive_scheduler: Progressive scheduler 인스턴스
            accumulation_steps: gradient accumulation 스텝 수
        ...
        # 현재 epoch의 hard negative ratio 계산
        self.current_hard_negative_ratio = self.progressive_scheduler.get_ratio(current_epoch)
        
        # Effective batch size 계산
        self.effective_batch_size = batch_size * accumulation_steps
        
        # Effective batch당 하드 네거티브 샘플 수
        self.hard_negative_per_effective_batch = int(self.effective_batch_size * self.current_hard_negative_ratio)
        self.random_per_effective_batch = self.effective_batch_size - self.hard_negative_per_effective_batch
        ...
        print(f"Progressive Sampler Configuration (Epoch {current_epoch}):")
        print(f"  - Batch size: {batch_size}")
        print(f"  - Accumulation steps: {accumulation_steps}")
        print(f"  - Effective batch size: {self.effective_batch_size}")
        print(f"  - Current hard negative ratio: {self.current_hard_negative_ratio:.3f}")
        print(f"  - Hard negative per effective batch: {self.hard_negative_per_effective_batch}")
        print(f"  - Random samples per effective batch: {self.random_per_effective_batch}")
        print(f"  - Samples per class per effective batch: {self.random_per_effective_batch // num_classes}")
        
    def update_epoch(self, epoch):
        """새로운 epoch에 대해 hard negative ratio 업데이트"""
        self.current_epoch = epoch
        self.current_hard_negative_ratio = self.progressive_scheduler.get_ratio(epoch)
        self.hard_negative_per_effective_batch = int(self.effective_batch_size * self.current_hard_negative_ratio)
        self.random_per_effective_batch = self.effective_batch_size - self.hard_negative_per_effective_batch
        
        print(f"Updated Progressive Sampler for Epoch {epoch}:")
        print(f"  - Hard negative ratio: {self.current_hard_negative_ratio:.3f}")
        print(f"  - Hard negative per effective batch: {self.hard_negative_per_effective_batch}")
        print(f"  - Random samples per effective batch: {self.random_per_effective_batch}")
        
    def _get_balanced_samples_for_effective_batch(self, hard_indices_set, class_positions):
        """Effective batch에 대해 클래스별로 균등하게 샘플 선택"""
        if self.random_per_effective_batch == 0:
            return []
    ...
    def __iter__(self):
        ...
        for effective_batch_idx in range(self.num_effective_batches):
            # Effective batch에 대해 하드 네거티브 샘플 선택
            hard_indices = []
            if self.hard_negative_per_effective_batch > 0:
                hard_indices = self.hard_negative_miner.get_hard_negatives(self.hard_negative_per_effective_batch)
            hard_indices_set = set(hard_indices)
...
# 사용 예시를 위한 수정된 데이터로더 생성 함수
def create_progressive_train_loader(train_dataset, hard_negative_miner, labels, num_classes, 
                                  batch_size, accumulation_steps, progressive_scheduler,
                                  current_epoch=0, num_workers=16, prefetch_factor=4, pin_memory=True):
    """
    Progressive hard negative ratio를 사용하는 train loader 생성
    """
    progressive_batch_sampler = ProgressiveBalancedHardNegativeBatchSampler(
        dataset_size=len(train_dataset),
        batch_size=batch_size,
        hard_negative_miner=hard_negative_miner,
        labels=labels,
    ...
    train_loader = DataLoader(
        train_dataset, 
        batch_sampler=progressive_batch_sampler,
        num_workers=num_workers, 
        pin_memory=pin_memory,
        prefetch_factor=prefetch_factor
    )
    
    return train_loader, progressive_batch_sampler
